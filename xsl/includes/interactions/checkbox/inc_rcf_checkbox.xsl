...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="checkbox[not(@displayType='itemSelect')]">
			...
			<xsl:for-each select="item">
				<xsl:element name="{$listItemElementName}">
					...
					<xsl:variable name="listItemId">
						...
						_checkbox_<xsl:value-of select="ancestor::checkbox/@id"/>_listItem_<xsl:value-of select="@id"/>
					</xsl:variable>

					<span class="{normalize-space(concat('markable checkbox ',$itemMarkedClass, ' ', $itemContentClasses))}">
	...
	<!-- rcf_checkboxlist -->
	<xsl:template match="checkbox[not(@displayType='itemSelect')]" mode="getRcfClassName">
		rcfCheckbox
	</xsl:template>
