...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="verticalCrossword[not(@example='y')]" mode="outputAnswers">
        ...
        , {
            ...
            "value": [
                <xsl:for-each select="word[not(@example='y')]">{
                    "elm": "<xsl:value-of select="@id"/>",
                    "type": "simple",
                    "caseSensitive": "n",
                    "value": "<xsl:value-of select="translate(normalize-space(text()), $lowercase, $uppercase)"/>"
