...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<xsl:template match="matching[not(@example='y')]" mode="outputAnswers">
		...
		,
		{
			...
			"value": [
				<xsl:for-each select="pair[not(@example='y')]">{
				"elm": "<xsl:value-of select="matchTarget/@id"/>",
				"type": "simple",
				"value": "<xsl:value-of select="matchItem/@id"/>"
