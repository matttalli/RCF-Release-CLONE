...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<xsl:template match="positioning[not(@example='y')]" mode="outputAnswers">
		...
		,
		{
			...
			"value": [
				<xsl:for-each select="item">"<xsl:apply-templates mode="orderHash"/>"<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>
			]
		}
	</xsl:template>
</xsl:stylesheet>