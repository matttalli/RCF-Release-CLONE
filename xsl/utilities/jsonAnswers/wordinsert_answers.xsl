...
>
<!--
	handles the answer json generation for all *wordInsert* style interactions :
...
-->
	...
	<!-- match against non-example wordInsert or itemWordInsert in the xml for answers generation -->
	<xsl:template match="wordInsert[not(@example='y')] | itemWordInsert[not(@example='y')]" mode="outputAnswers">
		...
		-->
		<xsl:variable name="prefixValue"><xsl:apply-templates select="items/prefix" mode="outputWordInsertAnswer"/></xsl:variable>
		<xsl:variable name="suffixValue"><xsl:apply-templates select="items/suffix" mode="outputWordInsertAnswer"/></xsl:variable>
		<xsl:variable name="itemsValue"><xsl:apply-templates select="items/item" mode="outputWordInsertAnswer"/></xsl:variable>
		...
		,
		{
			...
			"value": [
				...
				<xsl:if test="alternativeAnswers">,
					<xsl:apply-templates select="alternativeAnswers/item" mode="outputWordInsertAnswer"/>
				</xsl:if>
			]
		}
	</xsl:template>

	<xsl:template match="wordInsert[count(items/item[@positionable='y'])=0]" mode="detectInvalidAnswers">
		,{
			"error" : "Error ! no positionable='y' items in this word insert interaction: '<xsl:value-of select="@id"/>'"
		}
	</xsl:template>

	<xsl:template match="itemWordInsert[count(items/item[@positionable='y'])=0]" mode="detectInvalidAnswers">
		,{
			"error" : "Error ! no positionable='y' items in this item based word insert interaction: '<xsl:value-of select="@id"/>'"
		}
	</xsl:template>

	<!-- match against any child items/item, items/prefix or items/suffix element with the expected mode -->
	<xsl:template match="items/item | items/prefix | items/suffix" mode="outputWordInsertAnswer">
		<xsl:variable name="textValue"><xsl:apply-templates select="." mode="outputWordInsertAnswerText"/></xsl:variable>
	...
	<xsl:template match="alternativeAnswers/item" mode="outputWordInsertAnswer">
	...
	<xsl:template match="text()" mode="outputWordInsertAnswerText">
		<xsl:copy/>
	</xsl:template>
